...
if __name__ == '__main__':
    ...
    parser.add_argument('--json_file', default='trainval2all.json',
    ...
    for img in tqdm(data['images']):
        ...
        f_txt = open('trainval.txt', 'a+')

        f_txt.write("\n%s " % ('/home/liuhaoming/yolo3-pytorch-master/VOCdevkit/VOC2007/JPEGImages/' + txt_name))
        for ann in data['annotations']:
            if ann['image_id'] == img_id:
                box = convert((img_width, img_height), ann["bbox"])
                # 写入txt，共5个字段
                # f_txt.write("%s,%s,%s,%s,%s,%s\n" % (
                #     '/data/exdark/images/' + txt_name, box[0], box[1], box[2], box[3], id_map[ann["category_id"]]))
                f_txt.write("%s,%s,%s,%s,%s " % (
                    box[0], box[1], box[2], box[3], id_map[ann["category_id"]]))
